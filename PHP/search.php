<?php
    ...
    $firstTerm = strtok($searchTerm, ' ');
    $lastTerm = substr($exploded[1], 0, strlen($exploded[1]));
    $sql = mysqli_query($conn, "SELECT * FROM users WHERE (username LIKE'%{$firstTerm}%' OR username LIKE'%{$firstTerm}%') OR (username LIKE'%{$firstTerm}%' AND username LIKE'%{$lastTerm}%')");
                        // AND NOT uniqueid = '{$_SESSION['session_id']}''");
    ...
    if(mysqli_num_rows($sql) > 0) {
        ...
        echo '<div class="null flex"><p class="text-italic">No contact found</p></div>';
    }
?>